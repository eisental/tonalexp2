...
export const questions_text = {
  first_question: (
    <p>
      באיזו מידה הדובר השני (המיוצג על ידי הצליל המסיים) מסכים עם דבריו של הדובר הראשון (המיוצג ע"י האקורדים החוזרים)?
      <br/>
...
export const intro_texts = [(
  <React.Fragment>
    ...
    <p>ניסוי זה ייערך בשני חלקים, כשהשני מתבצע כשבוע לאחר הראשון. יש להתחיל כל אחד משני חלקי הניסוי רק לאחר יצירת קשר עם הנסיינ/ית וקבלת אישור ממנה. ביצוע הניסוי ללא אישור הנסיין יגרום לפסילתו. את מועד החלק השני יש לתאם מראש עם הנסיינ/ית, וכאמור, להתחיל רק לאחר קבלת אישור.</p>
    <p>את תחילת הניסוי (שלב התרגול) תבצעו כשהנסיינ/ית נמצאת אתכם בקשר מקוון, וזמינ/ה לשאלותיכם/ן. לאחר שלב התרגול ינותק הקשר המקוון, אך הנסיינ/ית תהייה זמינה לפניותיכם במידה ותתעורר בעיה. אנא דווחו לנסיינ/ית על סיום הניסוי.</p>
    ...
    <p>במהלך הניסוי תוכלו לקחת הפסקות קצרות במידת הצורך, אך יש לבצע כל אחד משני החלקים ברצף, ולא לפצלו. במהלך הפסקה (אם נלקחה) השאירו את הניסוי פתוח.</p>
  ...
  <React.Fragment>
    <p>שלום - ברוכים הבאים לחלק השני של הניסוי.</p>
    <p>חלק זה יש לבצע במועד שתואם מראש עם הנסיינ/ית ורק לאחר קבלת אישור ממנה.</p>
    <p>בחלק זה שני ניסויים נפרדים. לאחר שתסיימו את הניסוי הראשון תצאו להפסקה בת עשר דקות. עם תום ההפסקה, צרו קשר עם הנסיינ/ית לקבלת הסבר. אין להתחיל בניסוי השני ללא אישור מהנסיינ/ית.</p>
    <p>משך מכסימלי של המפגש הנוכחי - 90 דקות (כולל הפסקה בת 10 דקות בין שני הניסויים). אין צורך למהר - משך זה מאפשר התנהלות נינוחה וקשובה, האזנות חוזרות, והפסקות קצרות, במידה ותזדקקו להן.</p>
  </React.Fragment>
)];
...
export const first_experiment_intro = [
  (
    <React.Fragment>
      ...
      <p>לכל אחד מן הקטעים שתשמעו שני חלקים: בחלק הראשון כמה אקורדים (אקורד - כמה צלילים המושמעים יחד), ובחלק השני - צליל בודד החוזר פעמיים. דמו ששני חלקים אלו הם שני ה"דוברים" בדיאלוג:</p>
      <p>הדובר הראשון (סדרת האקורדים) מצהיר או טוען דבר מה, והדובר השני (הצליל המסיים) מגיב על דבריו.</p>
  ...
  (
    <React.Fragment>
      ...
      <p>אם אינכם בטוחים בזיהוי, לחצו שוב. אם עדיין אינכם בטוחים - קראו לנסיין.</p>
      <p>אם זיהיתם, לחצו "המשך" למעבר למסך הבא.</p>
    </React.Fragment>
  )
];
...
export const first_experiment_training_info = (
  <React.Fragment>
    ...
    <p>כל השאלות שתישאלו מתייחסות לדובר השני בדיאלוג (המיוצג על ידי הצליל המסיים) - לתגובתו לדובר הראשון, לרגשותיו כלפיו, או למאפיינים שונים שלו. אף שאין לשאלות אלו תשובות "נכונות" או "שגויות" אובייקטיבית, נסו לדייק סובייקטיבית בתשובה - לבחור בתשובה המייצגת באופן המדויק ביותר את מה <strong>שמרגיש</strong> לכם "נכון" עבור הקטע ששמעתם.</p>
...
export const first_experiment_training_end = (
  <React.Fragment>
    ...
    <br/>
    <p>למעבר לניסוי עצמו - לחץ המשך.</p>
  </React.Fragment>
);
...
export const second_experiment_intro = (
  <React.Fragment>
    ...
    <p>באיזו מידה מתאים הצליל המסיים את הקטע (שכונה "הדובר השני" בחלקים הקודמים של הניסוי) לשמש כהמשך למוזיקה הקודמת להם (שכונתה "הדובר הראשון" בחלקים הקודמים של הניסוי)?</p>
    <p>9 - במידה רבה מאד; 1 - כלל לא (השתמשו בכל הסקאלה כדי לציין דרגות שונות של התאמה).</p>
    <p>אם הבנתם את השאלה - לחצו "המשך". אם משהו אינו ברור - התקשרו לנסיין.</p>
  </React.Fragment>
);

export const second_experiment_question = (
  <p>
    באיזו מידה מתאים הצליל המסיים את הקטע לשמש כהמשך למוזיקה הקודמת לו?
    <br/>
